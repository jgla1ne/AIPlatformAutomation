...
### **Deployment (all 4 scripts complete and tested)**

```bash
# 0. Nuclear cleanup — wipe any existing tenant data + unmount EBS (run as root)
sudo bash scripts/0-complete-cleanup.sh <TENANT_ID>
# Type: DELETE <TENANT_ID>  to confirm

# 1. Interactive setup wizard — collects all config, mounts EBS, generates platform.conf
bash scripts/1-setup-system.sh <TENANT_ID>

# 2. Deploy all containers from platform.conf
bash scripts/2-deploy-services.sh <TENANT_ID>

# 3. Post-deploy configuration + Mission Control health checks
bash scripts/3-configure-services.sh <TENANT_ID>
```

> Script 0 requires `sudo`. Scripts 1-3 run as your normal user (non-root by design).
>
> The `platform.conf` generated by Script 1 is the single source of truth.
> Scripts 2 and 3 read it; never edit `docker-compose.yml` directly.
...
Each script has a single responsibility. They communicate exclusively through
`/mnt/<TENANT_ID>/config/platform.conf` — the single source of truth.

```
Script 0  →  nuclear teardown  (root)
Script 1  →  interactive config wizard  →  platform.conf  (non-root)
Script 2  →  reads platform.conf  →  docker-compose.yml + containers  (non-root)
Script 3  →  health checks, credentials, Mission Control  (non-root)
```
...
```bash
sudo bash scripts/0-complete-cleanup.sh <TENANT_ID>
# Confirms with: DELETE <TENANT_ID>
#
# What it does:
#   • docker compose down --volumes for the tenant
#   • rm -rf /mnt/<TENANT_ID>/   (the entire tenant tree)
#   • umount /mnt/<TENANT_ID>    (always — EBS unmount is not flag-gated)
#   • docker network rm <TENANT_ID>-network
#
# Options: --dry-run  --containers-only
```

#### Script 1 — Interactive Setup Wizard

```bash
bash scripts/1-setup-system.sh <TENANT_ID>
# Optionally pre-seed from a saved template:
bash scripts/1-setup-system.sh <TENANT_ID> --template ~/.ai-platform-templates/<TENANT_ID>-template.conf
#
# What it collects (all interactive, with sensible defaults):
#   • Tenant identity (platform prefix, domain, admin email, organisation)
#   • EBS volume selection (lsblk-based, NVMe-aware, multi-tenant safe)
#   • Stack preset: MINIMAL / DEVELOPMENT / STANDARD / FULL / CUSTOM
#   • Service credentials: Postgres user/db override, Redis, OpenClaw username/password
#   • LLM gateway: LiteLLM, Bifrost, or direct Ollama
#   • Vector database: Qdrant, Weaviate, Chroma, Milvus
#   • TLS: Let's Encrypt / Manual / Self-signed / None + single HTTP→HTTPS redirect toggle
#   • API keys: OpenAI, Anthropic, Google, Groq, Cohere, HuggingFace, OpenRouter, Mammouth
#   • Proxy: Caddy or Nginx routing (path-based or subdomain)
#   • Google Drive ingestion via rclone
#   • Signal bot (bbernhard/signal-cli-rest-api, MODE=json-rpc)
#
# Output: /mnt/<TENANT_ID>/config/platform.conf  (chmod 600)
#         ~/.ai-platform-templates/<TENANT_ID>-template.conf  (optional)
#
# EBS safety rules:
#   • Device already at /mnt/<TENANT_ID>  →  reuse filesystem, skip format
#   • Device mounted at another /mnt/<X>  →  FORMAT BLOCKED (protects other tenant)
#   • Unmounted device                    →  format + mount + fstab entry
```
...
#
# What it does:
#   • Sources /mnt/<TENANT_ID>/config/platform.conf
#   • Calls prepare_data_dirs() — creates all data dirs, chowns to PUID:PGID
#   • Generates docker-compose.yml (no deprecated version: field)
#   • Generates Caddyfile, litellm/config.yaml, bifrost/config.yaml, rclone.conf
#   • docker compose up -d
#   • Waits for each enabled service to pass its health check
#
# Per-service user: directives (correct per official images):
#   postgres, redis         → NOT overridden  (they manage internal UIDs)
#   ollama                  → NOT overridden  (runs as root, /root/.ollama)
#   n8n                     → NOT overridden  (node uid 1000, /home/node)
#   flowise                 → NOT overridden  (root internally, /root/.flowise)
#   authentik               → NOT overridden  (manages own users)
#   signalbot               → NOT overridden  (internal user uid 1000)
#   all others              → user: "${PUID}:${PGID}"
#
# Data path: ALL host-side volumes are under /mnt/<TENANT_ID>/
#            Container-internal paths (right side) are image conventions
```
...
bash scripts/3-configure-services.sh <TENANT_ID>
#
# Default run: configure all services + full Mission Control health sweep
#
# Options:
#   --verify-only                  # health checks only, skip configuration
#   --health-check                 # alias for verify-only
#   --show-credentials             # print all service credentials
#   --rotate-keys [service]        # regenerate secrets
#   --restart [service]            # restart one container
#   --add [service]                # enable + deploy a new service
#   --remove [service]             # stop + disable a service
#
# Mission Control checks (modular, non-fatal warnings):
#   check_port_health()            # per-service HTTP/TCP health on configured ports
#   check_dns_resolution()         # A-record validates against server public IP
#   check_api_keys()               # live API calls to OpenAI/Anthropic/Google/Groq
#   check_rclone_credentials()     # JSON structure + rclone lsd connectivity
#   check_signal_pairing()         # Signal device pairing status + pairing instructions
```
...
After first deploy, pair your Signal number:
```bash
# Check pairing status
bash scripts/3-configure-services.sh <TENANT_ID> --verify-only

# Option A — link existing Signal account (scan QR)
curl -s "http://127.0.0.1:${SIGNALBOT_PORT}/v1/qrcodelink/+15551234567?device_name=ai-platform"

# Option B — register new number
curl -s -X POST "http://127.0.0.1:${SIGNALBOT_PORT}/v1/register/+15551234567"
# then verify the SMS code:
curl -s -X POST "http://127.0.0.1:${SIGNALBOT_PORT}/v1/register/+15551234567/verify/<CODE>"
```
...
### **🚨 FORMERLY OUTDATED PSEUDO-CODE**

> The section below this line used to contain pseudo-bash that did not match the
> actual scripts. It has been replaced above with accurate documentation.
> The architecture diagrams and principles earlier in this file remain authoritative.
