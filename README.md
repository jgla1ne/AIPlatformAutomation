...
### Keeping a Running Stack Current

All container images use `:latest` tags. A clean Script 2 re-run always pulls the newest image for every service. For running stacks (no full redeploy), Script 3 provides per-service and bulk update commands.

```bash
# Update a single service (pulls new image, recreates container, waits for health)
bash scripts/3-configure-services.sh <tenant_id> --update openclaw
bash scripts/3-configure-services.sh <tenant_id> --update openwebui
bash scripts/3-configure-services.sh <tenant_id> --update litellm

# Update all non-data services for a tenant (rolling, health-checked)
bash scripts/3-configure-services.sh <tenant_id> --update
bash scripts/3-configure-services.sh <tenant_id> --update all

# Update Ollama model weights (re-pulls the latest manifest for each configured tag)
bash scripts/3-configure-services.sh <tenant_id> --ollama-update
```

**Safety model:**
- `--update all` skips Postgres, Redis, MongoDB automatically (major version bumps corrupt on-disk format without a migration procedure)
- `--update postgres` (explicit single-service) warns and waits 5s before proceeding
- Each service is health-checked after the container is recreated; failures are reported but don't abort other updates
- `openclaw.json`, `litellm_config.yaml`, and other config files are NOT regenerated by `--update` — only the container image changes. Run Script 2 for a full config refresh.

**Multi-tenant batch update:**
```bash
# Update all tenants in sequence
for tenant in datasquiz client-a client-b; do
  echo "=== Updating ${tenant} ==="
  bash scripts/3-configure-services.sh "${tenant}" --update all
  bash scripts/3-configure-services.sh "${tenant}" --ollama-update
done
```

**Security update policy:**
- AI UI services (OpenClaw, OpenWebUI, LibreChat, AnythingLLM): update freely, low data risk
- LLM gateway/routing (LiteLLM, Ollama): update for model support; config preserved
- Automation (N8N, Dify, Flowise): update with `--update <service>` + check logs for migration errors
- Auth (Authentik): update promptly for security; run `ak migrate` inside container if you see 500 errors after update
- Data (Postgres, Redis, MongoDB): update manually with backup first; major version only during maintenance window
...
**`${DATA_DIR}/openclaw/home` is `drwx------ ubuntu (uid 1000)`** — the deploy user (jglaine, uid 1001) cannot read, list, or write this directory at all. All openclaw file operations must go through `docker exec $container node` or `docker run alpine` (both run as uid 1000 or root):
- `prepare_data_dirs()` writes `openclaw.json` to `/tmp` then copies via `docker run alpine`
- `--update-channels` uses `docker exec node` with env vars (no host python3)
- `--openclaw-pairs` uses `docker exec node` to read `devices/pending.json` and `paired.json`
- Do NOT create or edit openclaw files on the host path directly — writes go nowhere useful
...
- [x] `channels.*.dmPolicy: "open"` + `allowFrom: ["*"]` — respond to anyone without pairing
- [x] `channels.discord.guilds.*.requireMention: false` — respond without @mention
- [x] All file ops via `docker exec node` / `docker run alpine` (home vol is `700 ubuntu`)
- [x] Agent model points at LiteLLM (`openai/mammouth/<model>` or first available provider)
- [x] Port mapping `"${OPENCLAW_PORT}:${OPENCLAW_PORT}"` (0.0.0.0 default for Caddy access)
- [ ] Signal: scan QR at `https://signal.<BASE_DOMAIN>/v1/qrcodelink` after first deploy
- [ ] Telegram: valid BotFather token — if 401, regenerate → `--update-channels`
- [ ] Discord: Message Content Intent enabled in Developer Portal; bot invited to server
