...
- Supported strategies: `simple-shuffle`, `least-busy`, `usage-based-routing`, `cost-based-routing`, `latency-based-routing`, **`external-first`**
- `external-first` = CPU-only alias: sets `latency-based-routing` + adds `router_settings.fallbacks` mapping each Ollama model to cloud alternatives with a 25-second timeout. If Ollama exceeds 25s (expected on CPU), LiteLLM transparently retries with Mammouth → Groq.
- Script 2 auto-applies `external-first` strategy when `GPU_TYPE=none` at deploy time; GPU instances get `simple-shuffle`
- `LITELLM_ROUTING_STRATEGY` persisted in platform.conf so it survives redeploys
- Script: `3-configure-services.sh --litellm-routing external-first`
...
*Version: 1.0.1 | Last Updated: 2026-04-27*
...
- **Telegram**: Token validated against Telegram API at deploy time; invalid tokens seeded as `enabled: false`. Regenerate via BotFather → update `TELEGRAM_BOT_TOKEN` in platform.conf → `--update-channels`. **Users must send `/start` to the bot first** (Telegram policy: bots cannot initiate DMs).
- **Discord**: Two steps required: (1) enable **Message Content Intent** in [Discord Developer Portal → Bot → Privileged Gateway Intents](https://discord.com/developers/applications); (2) invite the bot to the server via `https://discord.com/oauth2/authorize?client_id=<CLIENT_ID>&permissions=8&scope=bot`. `channels unresolved` = bot not yet in server.
- Channel authentication failures are isolated — do not crash the gateway
- `--update-channels` (Script 3): rebuilds channels section from platform.conf, re-validates Telegram, uses `docker exec node` (home vol is `drwx------ ubuntu`), restarts container
- **Constraint**: Telegram requires valid BotFather token + users send /start first
- **Constraint**: Discord requires intents enabled AND bot invited to server via OAuth URL
...
- [x] `models.providers.openai.models` lists all LiteLLM models — GPU: Ollama first; CPU: cloud first with Ollama last labelled "(Local/CPU — slow)"
- [x] Default primary model: GPU→first Ollama; CPU→first Mammouth/Anthropic/Groq cloud model
- [x] `router_settings.fallbacks`: CPU instances auto-fallback Ollama→cloud after 25s timeout
...
| **Discord Intents + bot invite** | 4014 / channels unresolved | Intents enabled + bot invited via OAuth URL | ✅ Resolved |
| **LLM idle timeout on CPU** | Discord bot no response | `idleTimeoutSeconds: 0` (disabled); cloud model as default | ✅ Resolved |
| **CPU defaults to slow Ollama** | All services use local inference | `external-first` routing: cloud models first + Ollama→cloud fallbacks after 25s | ✅ Resolved |
| **Signal SSE probe 404** | Channel shown as failed | Added `/health` endpoint to sse-proxy.py | ✅ Resolved |
| **openclaw.json in /home/jglaine** | File not mounted into container | home vol is 700 ubuntu; all writes via docker exec node/docker run alpine | ✅ Resolved |
