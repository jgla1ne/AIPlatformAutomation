...
# --- Dynamic Postgres Initializer (Zero Hardcoded Values) ---
generate_postgres_init_script() {
    local init_script="${TENANT_DIR}/init-user-db.sh"
    log "INFO" "Generating dynamic Postgres initialization script..."
    
    cat > "$init_script" << 'EOF'
#!/bin/bash
set -euo pipefail

# Dynamic Postgres Database and User Initializer
# Generated by Script 2 - Zero hardcoded values

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
EOF

    # Add per-service database creation based on enabled services
    if [[ "${ENABLE_AUTHENTIK}" == "true" ]]; then
        cat >> "$init_script" << EOF
    -- Create Authentik database and user
    CREATE DATABASE ${AUTHENTIK_DB_NAME};
    CREATE USER ${AUTHENTIK_DB_USER} WITH ENCRYPTED PASSWORD '${AUTHENTIK_DB_PASS}';
...
EOF
    ...
    if [[ "${ENABLE_FLOWISE}" == "true" ]]; then
        cat >> "$init_script" << EOF
    -- Create Flowise database and user
    CREATE DATABASE ${FLOWISE_DB_NAME};
    CREATE USER ${FLOWISE_DB_USER} WITH ENCRYPTED PASSWORD '${FLOWISE_DB_PASS}';
    GRANT ALL PRIVILEGES ON DATABASE ${FLOWISE_DB_NAME} TO ${FLOWISE_DB_USER};
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO ${FLOWISE_DB_USER};
    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO ${FLOWISE_DB_USER};
EOF
    fi
    ...
    CREATE USER ${N8N_DB_USER} WITH ENCRYPTED PASSWORD '${N8N_DB_PASS}';
...
EOF
    ...
    chmod +x "$init_script"
    chown "${TENANT_UID}:${TENANT_GID}" "$init_script"
    
    ok "Dynamic Postgres initialization script generated."
}

# --- Dynamic LiteLLM Config Generator (Zero Hardcoded Values) ---
generate_litellm_config() {
    local config_file="${TENANT_DIR}/litellm-config.yaml"
    log "INFO" "Generating LiteLLM config from high-level flags..."
    
    # Start with YAML structure
    cat > "$config_file" << 'EOF'
model_list:
EOF

    # Read the simple flag from .env and build the complex file
    local enabled_models="${LITELLM_ENABLED_MODELS}"
    
    # Split comma-separated models and add each
    IFS=',' read -ra MODELS <<< "$enabled_models"
    for model in "${MODELS[@]}"; do
        case "$model" in
            "ollama")
                cat >> "$config_file" << EOF
  - model_name: ollama-${OLLAMA_DEFAULT_MODEL:-mistral}
    litellm_params:
      model: ollama/${OLLAMA_DEFAULT_MODEL:-mistral}
      api_base: http://${OLLAMA_SERVICE_NAME:-ollama}:${OLLAMA_PORT:-11434}
      rpm_limit: 6
EOF
                ;;
            "gemini")
...
EOF
    ...
    chown "${TENANT_UID}:${TENANT_GID}" "$config_file"
    
    ok "Dynamic LiteLLM configuration generated from flags."
}
...
add_postgres() {
  ...
  postgres:
    ...
    volumes:
      ...
      # CRITICAL: Mount dynamic initialization script
...
add_litellm() {
  ...
  litellm:
    ...
    volumes:
      # Mount the dynamically generated config file
...
main() {
    ...
    # --- Generate Dynamic Postgres Initialization Script ---
    if [[ "${ENABLE_POSTGRES}" == "true" ]]; then
        generate_postgres_init_script
    fi
    
    # --- Generate Dynamic LiteLLM Configuration ---
    if [[ "${ENABLE_LITELM}" == "true" ]]; then
        generate_litellm_config
    fi
