...
# Map tenant variables to running variables
RUNNING_UID="${TENANT_UID:-1001}"
RUNNING_GID="${TENANT_GID:-1001}"
# ────────────────────────────────────────────────────────────────────
...
# Docker networks are created automatically by compose

# AppArmor profiles are optional — only load if present
load_apparmor_profiles() {
    ...
    if [[ -d "${profile_dir}" ]]; then
        for profile_path in "${profile_dir}"/aip-*; do
            [[ -f "$profile_path" ]] || continue
            if apparmor_parser -r "$profile_path" 2>/dev/null; then
                log "✅ AppArmor profile loaded: $(basename $profile_path)"
            else
                log "⚠️  AppArmor profile failed to load: $(basename $profile_path)"
            fi
        done
    else
        log "ℹ️  No AppArmor profiles directory found — skipping (this is fine)"
    fi
}
...
deploy_postgres() {
  ...
  if ! wait_for_service "postgres" \
    "$COMPOSE exec -T postgres pg_isready -U ${POSTGRES_USER} -d ${POSTGRES_DB}"; then
    FAILED_SERVICES+=("postgres")
    docker logs --tail 50 "${COMPOSE_PROJECT_NAME}-postgres" 2>&1 | tail -20
  fi
...
deploy_redis() {
  ...
  if ! wait_for_service "redis" \
    "$COMPOSE exec -T redis redis-cli -a ${REDIS_PASSWORD} ping"; then
    FAILED_SERVICES+=("redis")
    docker logs --tail 50 "${COMPOSE_PROJECT_NAME}-redis" 2>&1 | tail -20
  fi
}
...
deploy_qdrant() {
  ...
  if ! wait_for_service "qdrant" \
    "$COMPOSE exec -T qdrant curl -sf http://localhost:6333/health"; then
    FAILED_SERVICES+=("qdrant")
    docker logs --tail 50 "${COMPOSE_PROJECT_NAME}-qdrant" 2>&1 | tail -20
  fi
}
...
wait_for_service() {
  ...
  local max_wait=${3:-300}  # Increased to 5 minutes
  local interval=5
  local elapsed=0
...
main() {
    ...
    # Pre-flight volume permission fix
    log "Fixing volume ownership before deploy..."
    chown -R "${TENANT_UID}:${TENANT_GID}" "${DATA_ROOT}/data"
    [[ -d "${DATA_ROOT}/.gdrive" ]] && chown -R "${TENANT_UID}:${TENANT_GID}" "${DATA_ROOT}/.gdrive"
    [[ -d "${DATA_ROOT}/config" ]] && chown -R "${TENANT_UID}:${TENANT_GID}" "${DATA_ROOT}/config"
    [[ -d "${DATA_ROOT}/data/postgres" ]] && chmod 700 "${DATA_ROOT}/data/postgres"
    log "✅ Volume ownership confirmed"
    ...
    if [[ ${#FAILED_SERVICES[@]} -gt 0 ]]; then
        log "⚠️  The following services failed to start:"
        for svc in "${FAILED_SERVICES[@]}"; do
            log "   ❌ ${svc}"
            docker logs --tail 50 "${COMPOSE_PROJECT_NAME}-${svc}" 2>&1 | tail -20
        done
        log "Check logs above. Run script 3 anyway to configure what did start."
        exit 1
    fi
