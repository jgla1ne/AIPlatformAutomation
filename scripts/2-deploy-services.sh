...
generate_caddyfile() {
    ...
    mkdir -p "${CONFIG_DIR}/caddy" "${CONFIG_DIR}/caddy/conf.d"

    # Main Caddyfile: global settings + import conf.d/*.caddy
    # Each domain lives in its own conf.d file so a syntax error in one
    # file doesn't affect others — caddy reload validates before applying.
    cat > "${CONFIG_DIR}/caddy/Caddyfile" << EOF
# Global settings only — do not add site blocks here.
# Domain blocks live in conf.d/*.caddy (one file per tenant/domain).
# Safety: caddy reload validates the full config; old config stays on error.
{
    admin :2019
    email ${PROXY_EMAIL}
    log {
...
EOF
    ...
    # Tenant config goes into conf.d/tenant-${TENANT_ID}.caddy
    local _tenant_caddy="${CONFIG_DIR}/caddy/conf.d/tenant-${TENANT_ID}.caddy"
    cat > "${_tenant_caddy}" << EOF
# Tenant: ${TENANT_ID} — ${BASE_DOMAIN}
# Generated by Script 2. Managed separately from the control-plane config.
...
EOF
    ...
    ok "Caddyfile generated (tenant config: ${_tenant_caddy})"
}
