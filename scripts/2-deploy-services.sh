...
prepare_data_dirs() {
    ...
    if [[ "${OPENCLAW_ENABLED}" == "true" ]]; then
        mkdir -p "${DATA_DIR}/openclaw/data" "${DATA_DIR}/openclaw/config"
        # Write openclaw gateway config to allow all origins in the Control UI.
        # This JSON is mounted at /.openclaw/ inside the container.
        local _openclaw_origin="*"
        if [[ "${CADDY_ENABLED:-false}" == "true" && -n "${BASE_DOMAIN:-}" ]]; then
            _openclaw_origin="https://openclaw.${BASE_DOMAIN}"
        fi
        cat > "${DATA_DIR}/openclaw/config/config.json" << OCEOF
{
  "gateway": {
    "controlUi": {
...
OCEOF
        chmod 644 "${DATA_DIR}/openclaw/config/config.json"
    fi
