...
deploy_postgres() {
    print_info "Deploying PostgreSQL using generated compose template..."
    ...
    # Use compose template generated by Script 1 (with user mapping)
    if [[ ! -f "$COMPOSE_DIR/postgres/docker-compose.yml" ]]; then
        print_error "PostgreSQL compose template not found. Run Script 1 first."
        return 1
    fi
    
    print_success "PostgreSQL configuration found"
    echo "DEBUG: PostgreSQL configuration loaded" >> "$LOG_FILE"
...
EOF
    ...
    # 🔥 UPDATED: Use Script 1 generated compose template
deploy_ollama() {
    print_info "Deploying Ollama using generated compose template..."
    echo "$(date '+%Y-%m-%d %H:%M:%S') - Ollama configuration loaded" >> "$LOG_FILE"
    
    # Use compose template generated by Script 1 (with user mapping)
    if [[ ! -f "$COMPOSE_DIR/ollama/docker-compose.yml" ]]; then
        print_error "Ollama compose template not found. Run Script 1 first."
        return 1
    fi
