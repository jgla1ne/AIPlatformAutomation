...
main() {
    ...
    load_env_or_default() {
        ...
        # Container name defaults for cleanup
        LLM_GATEWAY_CONTAINER="${LLM_GATEWAY_CONTAINER:-ai-${TENANT}-bifrost-1}"
        MEM0_CONTAINER="${MEM0_CONTAINER:-${COMPOSE_PROJECT_NAME}_mem0}"
        OLLAMA_CONTAINER="${OLLAMA_CONTAINER:-${COMPOSE_PROJECT_NAME}_ollama}"
        QDRANT_CONTAINER="${QDRANT_CONTAINER:-${COMPOSE_PROJECT_NAME}_qdrant}"
        POSTGRES_CONTAINER="${POSTGRES_CONTAINER:-${COMPOSE_PROJECT_NAME}_postgres}"
        REDIS_CONTAINER="${REDIS_CONTAINER:-${COMPOSE_PROJECT_NAME}_redis}"
        N8N_CONTAINER="${N8N_CONTAINER:-${COMPOSE_PROJECT_NAME}_n8n}"
        FLOWISE_CONTAINER="${FLOWISE_CONTAINER:-${COMPOSE_PROJECT_NAME}_flowise}"
