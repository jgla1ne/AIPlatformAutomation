...
collect_configurations() {
    ...
    CODE_EXECUTION_API_KEY="${CODE_EXECUTION_API_KEY:-$(openssl rand -hex 32)}"
...
MINIO_ROOT_USER=${MINIO_ROOT_USER:-ds-admin}
...
ANYTHINGLLM_PORT=${ANYTHINGLLM_PORT:-5004}
OLLAMA_PORT=${OLLAMA_PORT:-11434}
QDRANT_PORT=${QDRANT_PORT:-6333}
MINIO_PORT=${MINIO_PORT:-9000}
MINIO_CONSOLE_PORT=${MINIO_CONSOLE_PORT:-9001}
DIFY_PORT=${DIFY_PORT:-5003}
DIFY_WEB_PORT=${DIFY_WEB_PORT:-3002}
SIGNAL_API_PORT=${SIGNAL_API_PORT:-8085}
OPENCLAW_PORT=${OPENCLAW_PORT:-18789}
TAILSCALE_PORT=${TAILSCALE_PORT:-8443}
LITELLM_PORT=${LITELLM_PORT:-5005}
OPENWEBUI_PORT=${OPENWEBUI_PORT:-5006}
...
allocate_port() {
    ...
    if [[ " ${selected_services[*]} " =~ " dify " ]]; then
        echo "DIFY_SECRET_KEY=$(openssl rand -hex 32)" >> "$ENV_FILE"
    fi
...
create_tenant_directories() {
    ...
    local dirs=(
        ...
        "${TENANT_DIR}/config/tailscale"
        "${TENANT_DIR}/data/openclaw"
        "${TENANT_DIR}/data/postgres"
        "${TENANT_DIR}/data/redis"
        "${TENANT_DIR}/data/qdrant"
        "${TENANT_DIR}/data/minio"
        "${TENANT_DIR}/data/n8n"
        "${TENANT_DIR}/data/signal"
        "${TENANT_DIR}/data/flowise"
        "${TENANT_DIR}/data/anythingllm"
        "${TENANT_DIR}/data/dify/db"
        "${TENANT_DIR}/data/dify/redis"
        "${TENANT_DIR}/data/dify/storage"
    ...
    chmod 700 "${TENANT_DIR}/data/postgres"
    chmod 700 "${TENANT_DIR}/config"
...
main() {
    ...
    mkdir -p "${DATA_ROOT}/logs" 2>/dev/null || echo "DEBUG: Failed to create ${DATA_ROOT}/logs"
    export LOG_FILE="${DATA_ROOT}/logs/setup.log"
