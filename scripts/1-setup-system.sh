...
configure_network() {
    ...
    read -p "Do you have a domain name for this installation? (y/n): " has_domain
...
configure_firewall() {
    ...
    if command -v ufw &> /dev/null; then
        ...
        # Allow service ports if no domain
        if [ "${CONFIG["HAS_DOMAIN"]}" = "false" ]; then
            [ "${CONFIG["INSTALL_N8N"]}" = "true" ] && ufw allow ${CONFIG["N8N_PORT"]}/tcp comment 'n8n'
            [ "${CONFIG["INSTALL_FLOWISE"]}" = "true" ] && ufw allow ${CONFIG["FLOWISE_PORT"]}/tcp comment 'Flowise'
            [ "${CONFIG["INSTALL_OPENWEBUI"]}" = "true" ] && ufw allow ${CONFIG["OPENWEBUI_PORT"]}/tcp comment 'Open WebUI'
            [ "${CONFIG["INSTALL_ANYTHINGLLM"]}" = "true" ] && ufw allow ${CONFIG["ANYTHINGLLM_PORT"]}/tcp comment 'AnythingLLM'
            [ "${CONFIG["INSTALL_DIFY"]}" = "true" ] && ufw allow ${CONFIG["DIFY_PORT"]}/tcp comment 'Dify'
            [ "${CONFIG["INSTALL_LIBRECHAT"]}" = "true" ] && ufw allow ${CONFIG["LIBRECHAT_PORT"]}/tcp comment 'LibreChat'
            [ "${CONFIG["INSTALL_LANGFUSE"]}" = "true" ] && ufw allow ${CONFIG["LANGFUSE_PORT"]}/tcp comment 'Langfuse'
            [ "${CONFIG["INSTALL_UPTIME_KUMA"]}" = "true" ] && ufw allow ${CONFIG["UPTIME_KUMA_PORT"]}/tcp comment 'Uptime Kuma'
            [ "${CONFIG["INSTALL_PORTAINER"]}" = "true" ] && ufw allow ${CONFIG["PORTAINER_PORT"]}/tcp comment 'Portainer'
            [ "${CONFIG["INSTALL_COMFYUI"]}" = "true" ] && ufw allow ${CONFIG["COMFYUI_PORT"]}/tcp comment 'ComfyUI'
            [ "${CONFIG["INSTALL_OPENCLAW"]}" = "true" ] && ufw allow ${CONFIG["OPENCLAW_PORT"]}/tcp comment 'OpenClaw'
        fi
        ...
        # Allow service ports if no domain
        if [ "${CONFIG["HAS_DOMAIN"]}" = "false" ]; then
            [ "${CONFIG["INSTALL_N8N"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["N8N_PORT"]}/tcp
            [ "${CONFIG["INSTALL_FLOWISE"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["FLOWISE_PORT"]}/tcp
            [ "${CONFIG["INSTALL_OPENWEBUI"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["OPENWEBUI_PORT"]}/tcp
            [ "${CONFIG["INSTALL_ANYTHINGLLM"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["ANYTHINGLLM_PORT"]}/tcp
            [ "${CONFIG["INSTALL_DIFY"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["DIFY_PORT"]}/tcp
            [ "${CONFIG["INSTALL_LIBRECHAT"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["LIBRECHAT_PORT"]}/tcp
            [ "${CONFIG["INSTALL_LANGFUSE"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["LANGFUSE_PORT"]}/tcp
            [ "${CONFIG["INSTALL_UPTIME_KUMA"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["UPTIME_KUMA_PORT"]}/tcp
            [ "${CONFIG["INSTALL_PORTAINER"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["PORTAINER_PORT"]}/tcp
            [ "${CONFIG["INSTALL_COMFYUI"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["COMFYUI_PORT"]}/tcp
            [ "${CONFIG["INSTALL_OPENCLAW"]}" = "true" ] && firewall-cmd --permanent --add-port=${CONFIG["OPENCLAW_PORT"]}/tcp
        fi
...
verify_services() {
    ...
    while [ $wait_time -lt $max_wait ]; do
        ...
        for service in $(docker compose ps --services); do
            local status=$(docker compose ps "$service" --format json | jq -r '.[0].Health // "none"')
...
setup_monitoring() {
    if [ "${CONFIG["INSTALL_UPTIME_KUMA"]}" = "true" ]; then
        print_section "Uptime Kuma Setup"
        
        echo ""
        echo "Uptime Kuma has been installed. To complete setup:"
        echo "1. Access Uptime Kuma at:"
        if [ "${CONFIG["HAS_DOMAIN"]}" = "true" ]; then
            echo "   https://${CONFIG["UPTIME_KUMA_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}"
        else
            echo "   http://${CONFIG["PUBLIC_IP"]}:${CONFIG["UPTIME_KUMA_PORT"]}"
        fi
...
setup_portainer() {
    if [ "${CONFIG["INSTALL_PORTAINER"]}" = "true" ]; then
        print_section "Portainer Setup"
        
        echo ""
        echo "Portainer has been installed. To complete setup:"
        echo "1. Access Portainer at:"
        if [ "${CONFIG["HAS_DOMAIN"]}" = "true" ]; then
            echo "   https://${CONFIG["PORTAINER_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}"
        else
            echo "   https://${CONFIG["PUBLIC_IP"]}:${CONFIG["PORTAINER_PORT"]}"
        fi
        echo "2. Create your admin account (within 5 minutes)"
...
generate_access_info() {
    ...
    local access_file="${PROJECT_ROOT}/SERVICE_ACCESS.md"
    
    cat > "$access_file" << EOF
# AI Platform Service Access Information
Generated: $(date)
...
EOF
        [ "${CONFIG["INSTALL_N8N"]}" = "true" ] && echo "- **n8n**: https://${CONFIG["N8N_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_FLOWISE"]}" = "true" ] && echo "- **Flowise**: https://${CONFIG["FLOWISE_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_OPENWEBUI"]}" = "true" ] && echo "- **Open WebUI**: https://${CONFIG["OPENWEBUI_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_ANYTHINGLLM"]}" = "true" ] && echo "- **AnythingLLM**: https://${CONFIG["ANYTHINGLLM_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_DIFY"]}" = "true" ] && echo "- **Dify**: https://${CONFIG["DIFY_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_LIBRECHAT"]}" = "true" ] && echo "- **LibreChat**: https://${CONFIG["LIBRECHAT_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_LANGFUSE"]}" = "true" ] && echo "- **Langfuse**: https://${CONFIG["LANGFUSE_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_UPTIME_KUMA"]}" = "true" ] && echo "- **Uptime Kuma**: https://${CONFIG["UPTIME_KUMA_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_PORTAINER"]}" = "true" ] && echo "- **Portainer**: https://${CONFIG["PORTAINER_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_COMFYUI"]}" = "true" ] && echo "- **ComfyUI**: https://${CONFIG["COMFYUI_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_OPENCLAW"]}" = "true" ] && echo "- **OpenClaw**: https://${CONFIG["OPENCLAW_SUBDOMAIN"]}.${CONFIG["DOMAIN"]}" >> "$access_file"
    else
        cat >> "$access_file" << EOF
Services are accessible via HTTP on their respective ports.

EOF
        [ "${CONFIG["INSTALL_N8N"]}" = "true" ] && echo "- **n8n**: http://${CONFIG["PUBLIC_IP"]}:${CONFIG["N8N_PORT"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_FLOWISE"]}" = "true" ] && echo "- **Flowise**: http://${CONFIG["PUBLIC_IP"]}:${CONFIG["FLOWISE_PORT"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_OPENWEBUI"]}" = "true" ] && echo "- **Open WebUI**: http://${CONFIG["PUBLIC_IP"]}:${CONFIG["OPENWEBUI_PORT"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_ANYTHINGLLM"]}" = "true" ] && echo "- **AnythingLLM**: http://${CONFIG["PUBLIC_IP"]}:${CONFIG["ANYTHINGLLM_PORT"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_DIFY"]}" = "true" ] && echo "- **Dify**: http://${CONFIG["PUBLIC_IP"]}:${CONFIG["DIFY_PORT"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_LIBRECHAT"]}" = "true" ] && echo "- **LibreChat**: http://${CONFIG["PUBLIC_IP"]}:${CONFIG["LIBRECHAT_PORT"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_LANGFUSE"]}" = "true" ] && echo "- **Langfuse**: http://${CONFIG["PUBLIC_IP"]}:${CONFIG["LANGFUSE_PORT"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_UPTIME_KUMA"]}" = "true" ] && echo "- **Uptime Kuma**: http://${CONFIG["PUBLIC_IP"]}:${CONFIG["UPTIME_KUMA_PORT"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_PORTAINER"]}" = "true" ] && echo "- **Portainer**: https://${CONFIG["PUBLIC_IP"]}:${CONFIG["PORTAINER_PORT"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_COMFYUI"]}" = "true" ] && echo "- **ComfyUI**: http://${CONFIG["PUBLIC_IP"]}:${CONFIG["COMFYUI_PORT"]}" >> "$access_file"
        [ "${CONFIG["INSTALL_OPENCLAW"]}" = "true" ] && echo "- **OpenClaw**: http://${CONFIG["PUBLIC_IP"]}:${CONFIG["OPENCLAW_PORT"]}" >> "$access_file"
    fi
...
EOF
    ...
    [ "${LLM_PROVIDERS["OPENAI"]}" = "true" ] && echo "- ✓ OpenAI configured" >> "$access_file" || echo "- ✗ OpenAI not configured" >> "$access_file"
    [ "${LLM_PROVIDERS["ANTHROPIC"]}" = "true" ] && echo "- ✓ Anthropic configured" >> "$access_file" || echo "- ✗ Anthropic not configured" >> "$access_file"
    [ "${LLM_PROVIDERS["GROQ"]}" = "true" ] && echo "- ✓ Groq configured" >> "$access_file" || echo "- ✗ Groq not configured" >> "$access_file"
    [ "${LLM_PROVIDERS["MISTRAL"]}" = "true" ] && echo "- ✓ Mistral configured" >> "$access_file" || echo "- ✗ Mistral not configured" >> "$access_file"
    [ "${LLM_PROVIDERS["COHERE"]}" = "true" ] && echo "- ✓ Cohere configured" >> "$access_file" || echo "- ✗ Cohere not configured" >> "$access_file"
    [ "${LLM_PROVIDERS["TOGETHER"]}" = "true" ] && echo "- ✓ Together AI configured" >> "$access_file" || echo "- ✗ Together AI not configured" >> "$access_file"
    [ "${LLM_PROVIDERS["PERPLEXITY"]}" = "true" ] && echo "- ✓ Perplexity configured" >> "$access_file" || echo "- ✗ Perplexity not configured" >> "$access_file"
    [ "${LLM_PROVIDERS["OPENROUTER"]}" = "true" ] && echo "- ✓ OpenRouter configured" >> "$access_file" || echo "- ✗ OpenRouter not configured" >> "$access_file"
    [ "${LLM_PROVIDERS["GOOGLE"]}" = "true" ] && echo "- ✓ Google AI configured" >> "$access_file" || echo "- ✗ Google AI not configured" >> "$access_file"
    [ "${LLM_PROVIDERS["XAI"]}" = "true" ] && echo "- ✓ xAI configured" >> "$access_file" || echo "- ✗ xAI not configured" >> "$access_file"
    [ "${LLM_PROVIDERS["HUGGINGFACE"]}" = "true" ] && echo "- ✓ HuggingFace configured" >> "$access_file" || echo "- ✗ HuggingFace not configured" >> "$access_file"
...
2. API keys and secrets are stored in \`.secrets.env\`
3. Keep \`.secrets.env\` secure and never commit to version control
4. Firewall is configured to allow only necessary ports
5. Fail2ban is active to prevent brute-force attacks
...
- Secrets: ${PROJECT_ROOT}/.secrets.env (keep secure!)

---
Generated by AI Platform Installer v${CONFIG["SCRIPT_VERSION"]}
EOF
...
cd "$(dirname "$0")"
docker compose up -d
echo "Services started. Run './status.sh' to check status."
EOF
...
docker compose up -d
echo "Update complete. Run './status.sh' to check status."
EOF
...
docker compose exec -T postgres pg_dumpall -U postgres > "$BACKUP_DIR/database.sql"
...
main() {
    ...
    # Setup reverse proxy if domain configured
    if [ "${CONFIG["HAS_DOMAIN"]}" = "true" ]; then
        generate_nginx_config
        add_nginx_to_compose
        setup_ssl_certificates
    ...
    echo ""
    echo "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo "${GREEN}  ✓ All services have been deployed successfully!${NC}"
    echo "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo "📋 Service access information: ${PROJECT_ROOT}/SERVICE_ACCESS.md"
    echo "📁 Project directory: ${PROJECT_ROOT}"
    echo "📝 Configuration file: ${PROJECT_ROOT}/.env"
    echo "🔐 Secrets file: ${PROJECT_ROOT}/.secrets.env"
    ...
    echo "   ./backup.sh   - Backup data and configuration"
    echo ""
    
    if [ "${CONFIG["HAS_DOMAIN"]}" = "false" ]; then
        echo "⚠️  Services are running on HTTP. Consider configuring a domain with SSL."
        echo ""
    fi
    
    echo "📚 Documentation: https://github.com/yourusername/ai-platform"
    echo "💬 Support: https://github.com/yourusername/ai-platform/issues"
    echo ""
    echo "${GREEN}Thank you for using the AI Platform Installer!${NC}"
    echo ""
