#!/usr/bin/env bash
# =============================================================================
# Script 1: System Setup Wizard - STABLE v5.0 (Definitive Reconstruction)
# =============================================================================
...
POSTGRES_PORT=5432; REDIS_PORT=6379; OLLAMA_PORT=11434; QDRANT_PORT=6333;
...
# --- Step 2: Reverse Proxy Selection (Previously Missing) ---
...
# --- Step 3: Stack Selection (Now with OpenClaw) ---
select_stack() {
...
# --- Step 4: VectorDB Selection (Previously Missing) ---
...
# --- Step 5: Ollama Model Configuration (Previously Missing) ---
...
finalize_setup() {
    ...
    log "Applying final ownership..."
    chown -R "${TENANT_UID}:${TENANT_GID}" "${DATA_ROOT}"; if [[ -d "${DATA_ROOT}/grafana" ]]; then chown -R 472:472 "${DATA_ROOT}/grafana"; fi; if [[ -d "${DATA_ROOT}/prometheus-data" ]]; then chown -R 65534:65534 "${DATA_ROOT}/prometheus-data"; fi; ok "Ownership set."
    read -p "Deploy services now? [Y/n]: " run_next; if [[ "${run_next:-y}" =~ ^y$ ]]; then log "Executing deployment..."; bash "$(dirname "${BASH_SOURCE[0]}")/2-deploy-services.sh"; else log "Deployment skipped."; fi
}
