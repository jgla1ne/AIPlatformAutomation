...
configure_service_credentials() {
    ...
    if [[ "${ENABLE_POSTGRES:-false}" == "true" ]]; then
        ...
        echo ""
        # Per-service database names — each postgres-backed service gets its own DB
        # to prevent Alembic migration conflicts (e.g. Dify vs Zep sharing 'messages' table).
        echo "  📂 Per-service database names (each service gets a dedicated PostgreSQL database):"
        echo "     Leave blank to accept the default (shown in brackets)."
        local _pg_base="${POSTGRES_DB:-${TENANT_ID}}"
        [[ "${ENABLE_LITELLM:-false}"   == "true" ]] && safe_read "  LiteLLM DB name"   "${_pg_base}_litellm"   "LITELLM_DB_NAME"
        [[ "${ENABLE_N8N:-false}"       == "true" ]] && safe_read "  N8N DB name"        "${_pg_base}_n8n"       "N8N_DB_NAME"
        [[ "${ENABLE_ZEP:-false}"       == "true" ]] && safe_read "  Zep DB name"        "${_pg_base}_zep"       "ZEP_DB_NAME"
        [[ "${ENABLE_DIFY:-false}"      == "true" ]] && safe_read "  Dify DB name"       "${_pg_base}_dify"      "DIFY_DB_NAME"
        [[ "${ENABLE_AUTHENTIK:-false}" == "true" ]] && safe_read "  Authentik DB name"  "${_pg_base}_authentik" "AUTHENTIK_DB_NAME"
        [[ "${ENABLE_LETTA:-false}"     == "true" ]] && safe_read "  Letta DB name"      "${_pg_base}_letta"     "LETTA_DB_NAME"
        echo ""
    fi
...
# Per-service dedicated PostgreSQL database names (avoids cross-service Alembic migration conflicts).
# Defaults: ${POSTGRES_DB}_<service>. Overridable in the Script 1 wizard.
